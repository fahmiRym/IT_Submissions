        ...
        <div class="card stat-card text-white h-100" style="background: linear-gradient(135deg, #4f46e5 0%, #3730a3 100%);">
            ...
            <div class="card-body p-4 position-relative" style="z-index: 2;">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <span class="badge bg-white bg-opacity-20 rounded-pill">Total Manager</span>
                </div>
                <h2 class="fw-bold mb-1">{{ number_format($totalManager) }}</h2>
                <p class="mb-0 small opacity-75 text-uppercase fw-bold">Manager Terdaftar</p>
            </div>
        </div>
    </div>
    <div class="col-md-6">
        <div class="card stat-card text-white h-100" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%);">
            ...
            <div class="card-body p-4 position-relative" style="z-index: 2;">
                ...
                <h2 class="fw-bold mb-1 text-truncate">{{ $latestManager }}</h2>
...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
    <div>
        <h5 class="fw-bold mb-0">Daftar Manager</h5>
        <p class="text-muted small mb-0">Manajemen data pimpinan dan penanggung jawab</p>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover mb-0">
                ...
                <tbody>
                    ...
                    <tr class="{{ !$m->is_active ? 'bg-light opacity-75' : '' }}">
                        <td class="ps-4 text-muted fw-bold">{{ $loop->iteration }}</td>
                        <td>
                            <div class="d-flex align-items-center">
                                <div class="bg-primary bg-opacity-10 text-primary rounded-circle d-flex align-items-center justify-content-center me-3 shadow-sm text-primary fw-bold" style="width: 40px; height: 40px; font-size: 1rem;">
                                    {{ substr(strtoupper($m->name), 0, 1) }}
                                </div>
                                <div class="fw-bold text-dark {{ !$m->is_active ? 'text-decoration-line-through' : '' }}">{{ $m->name }}</div>
                            </div>
                        </td>
                        <td>
                            @if($m->is_active)
                        ...
                        <td class="text-center">
                            <div class="d-flex justify-content-center gap-2">
                                ...
                                <form action="{{ route('superadmin.managers.toggle', $m->id) }}" method="POST" class="d-inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" class="btn-icon {{ $m->is_active ? 'btn-delete' : 'btn-edit' }}" title="{{ $m->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                                        <i class="bi {{ $m->is_active ? 'bi-slash-circle' : 'bi-check-circle' }}"></i>
                                    </button>
                                </form>

                                <button class="btn-icon btn-edit" onclick="editManager('{{ $m->id }}', '{{ $m->name }}')" title="Edit">
                                    <i class="bi bi-pencil-fill"></i>
                                </button>
                                <form action="{{ route('superadmin.managers.destroy', $m->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus manager ini?')">
                                    @csrf @method('DELETE')
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow">
            <div class="modal-header bg-primary text-white border-0 py-3 mt-2 mx-2 rounded-4">
                <h5 class="modal-title fw-bold"><i class="bi bi-person-plus-fill me-2"></i>Tambah Manager Baru</h5>
            ...
            <form action="{{ route('superadmin.managers.store') }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-uppercase">Nama Lengkap Manager</label>
                        <input type="text" name="name" class="form-control bg-light border-0 py-2" placeholder="Masukkan nama manager..." required>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow">
            <div class="modal-header bg-warning text-dark border-0 py-3 mt-2 mx-2 rounded-4">
                <h5 class="modal-title fw-bold"><i class="bi bi-pencil-square me-2"></i>Edit Data Manager</h5>
...
<script>
    function editManager(id, name) {
        let url = "{{ route('superadmin.managers.update', ':id') }}";
