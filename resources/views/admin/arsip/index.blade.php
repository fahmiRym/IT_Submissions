...
@section('content')
                                    ...
                                    @if($a->updated_by)
                                        <div class="mt-2 pt-1 border-top border-light border-opacity-50">
                                            ...
                                            <div class="d-flex align-items-center gap-1">
                                                <span class="fw-bold text-secondary" style="font-size: 0.65rem;">{{ $a->editor->name ?? 'System' }}</span>
                                            </div>
                                    ...
                                    <div class="d-flex flex-column gap-1" style="max-width: 250px;">
                                        ...
                                        @if($a->keterangan && !$itemsFound)
                                            <div class="text-muted italic small text-truncate" style="max-width: 200px;">
                                                "{{ $a->keterangan }}"</div>
                                        @endif
                                        @if(($a->adjustItems->count() + $a->mutasiItems->count()) > 1)
                                    ...
                                    <div class="d-flex gap-2 justify-content-end align-items-center">
                                        ...
                                        <div class="btn-group shadow-sm rounded-3 overflow-hidden">
                                            ...
                                            <button class="btn btn-sm btn-light"
                                                ...
                                                style="width: 38px; height: 38px; display: flex; align-items: center; justify-content: center; background: white;">
                                                <i class="bi bi-eye {{ $a->bukti_scan ? 'text-info' : 'text-muted' }}"></i>
                                            </button>
                                        </div>

                                        {{-- TOMBOL EDIT (Jika masih bisa diedit) --}}
                                        ...
                                        {{-- KHUSUS ACCOUNTING: UPLOAD BA --}}
                                        @if(auth()->user()->role === 'accounting' && $a->jenis_pengajuan === 'Adjust' && in_array($a->ket_process, ['Process', 'Done']))
                                            <button class="btn btn-sm shadow-sm rounded-3 px-3 fw-bold text-white btn-upload-ba d-flex align-items-center gap-2"
                                                ...
                                                onclick="openModalReuploadBA({{ $a->id }}, '{{ $a->no_registrasi }}')"
                                        ...
                                        {{-- VIEW SCAN BA ACCOUNTING (Jika sudah ada) --}}
                                        @if($a->scan_ba_accounting)
                                            <button class="btn btn-sm btn-success shadow-sm rounded-3"
                                                onclick="showBukti('{{ url('/preview-file/' . $a->scan_ba_accounting) }}')"
    ...
    {{-- MODAL UPLOAD ULANG BA (KHUSUS ACCOUNTING) --}}
