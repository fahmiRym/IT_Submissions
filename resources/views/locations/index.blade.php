...
@section('title', 'Master Data Lokasi Fisik')
@section('page-title', '🏢 Master Lokasi (Physical)')
...
<div class="row g-4 mb-4">
    <div class="col-md-6">
        <div class="card-stat-vibrant h-100 p-4" style="background: linear-gradient(135deg, #818cf8, #4f46e5);">
            ...
            <h2 class="mb-0 fw-bold display-5 text-white">{{ number_format($totalLoc) }}</h2>
    ...
    <div class="col-md-6">
        <div class="card-stat-vibrant h-100 p-4" style="background: linear-gradient(135deg, #fbbf24, #d97706);">
            ...
            <h2 class="mb-0 fw-bold text-white text-truncate" style="font-size:2.2rem; line-height:1.2; padding-top:4px;" title="{{ $latestLoc }}">{{ \Illuminate\Support\Str::limit($latestLoc, 25) }}</h2>
        ...
        <div class="table-responsive">
            <table class="table mb-0">
                ...
                <tbody>
                    ...
                    <tr class="{{ !$l->is_active ? 'opacity-60' : '' }}">
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <span class="fw-bold {{ !$l->is_active ? 'text-decoration-line-through text-muted' : 'text-dark' }}">{{ $l->name }}</span>
                            </div>
                        </td>
                        <td>
                            @if($l->is_active)
                        ...
                        <td class="text-center pe-4">
                            <div class="d-flex justify-content-center gap-2">
                                <form action="{{ route('superadmin.locations.toggle', $l->id) }}" method="POST" class="d-inline">
                                    @csrf @method('PATCH')
                                    <button type="submit" title="{{ $l->is_active ? 'Nonaktifkan' : 'Aktifkan' }}"
                                        class="btn-act {{ $l->is_active ? 'btn-act-toggle-off' : 'btn-act-toggle-on' }}">
                                        <i class="bi {{ $l->is_active ? 'bi-slash-circle-fill' : 'bi-check-circle-fill' }}"></i>
                                    </button>
                                </form>
                                <button type="button" class="btn-act btn-act-edit" title="Edit" onclick="editLoc('{{ $l->id }}', '{{ addslashes($l->name) }}')">
                                    <i class="bi bi-pencil-fill"></i>
                                </button>
                                <form action="{{ route('superadmin.locations.destroy', $l->id) }}" method="POST" class="d-inline"
                                      onsubmit="return confirm('Hapus lokasi {{ addslashes($l->name) }}?')">
                                    @csrf @method('DELETE')
...
<script>
    function editLoc(id, name) {
        let url = "{{ route('superadmin.locations.update', ':id') }}".replace(':id', id);
