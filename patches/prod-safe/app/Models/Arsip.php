 ...
 * ----------------------------------------------------------------------------
 * Sumber baseline: `/root/it_submissions/app/Models/Arsip.php` di prod
 *                  per 2026-07-29 (verified via SSH).
 *
 * PATCH DIAPPLY dibanding baseline:
 * 1. Tambah `protected $appends = ['status_utama'];`
 * 2. Tambah accessor `getStatusUtamaAttribute()` — supaya field `status_utama`
 ...
 *   hasApprovalChain/ensureVerifyToken/signatureFor/toApiDetailArray`
 *   (butuh chain approval + digital signature tables)
 * - Method `isProdukBaru` (butuh migration `add_produk_baru_to_jenis_enum`)
 * - Kolom `tindakan/catatan_it/scan_ba_accounting/scan_final/updated_by` di
 *   fillable (kolom belum ada di prod)
 * - Param `$note` di `processArchiving` (butuh kolom `catatan_it`)
 * - Method `editor()` relation (butuh kolom `updated_by`)
 * - Method `getCopyAllTextAttribute` (feature enhancement, aman skip)
 *
 * DEPLOY: SCP file ini ke `/root/it_submissions/app/Models/Arsip.php`
...
class Arsip extends Model
{
     ...
     * Baca kolom `status` + `no_doc` yang SUDAH ada di prod baseline.
     */
