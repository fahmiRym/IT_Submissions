...
<div class="row g-4 mb-4">
    <div class="col-md-3">
        <div class="card-stat-vibrant h-100 p-4" style="background: linear-gradient(135deg, #4f46e5, #3730a3);">
            ...
            <h2 class="mb-0 fw-bold display-5 text-white"><?php echo e(number_format($users->count())); ?></h2>
    ...
    <div class="col-md-3">
        <div class="card-stat-vibrant h-100 p-4" style="background: linear-gradient(135deg, #0ea5e9, #0284c7);">
            ...
            <h2 class="mb-0 fw-bold display-5 text-white"><?php echo e(number_format($totalAdmin)); ?></h2>
    ...
    <div class="col-md-3">
        <div class="card-stat-vibrant h-100 p-4" style="background: linear-gradient(135deg, #ef4444, #dc2626);">
            ...
            <h2 class="mb-0 fw-bold display-5 text-white"><?php echo e(number_format($totalSuper)); ?></h2>
    ...
    <div class="col-md-3">
        <div class="card-stat-vibrant h-100 p-4" style="background: linear-gradient(135deg, #10b981, #059669);">
            ...
            <h2 class="mb-0 fw-bold text-white text-truncate" style="font-size:2.2rem; line-height:1.2; padding-top:4px;"><?php echo e($latestUser ?: '-'); ?></h2>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-3">
                                <?php if($u->photo && file_exists(public_path('profile_photos/' . $u->photo))): ?>
                                    <img src="<?php echo e(asset('profile_photos/' . $u->photo)); ?>" alt="<?php echo e($u->name); ?>"
                                         class="avatar-user rounded-circle" style="object-fit:cover;border-radius:50%!important;">
                                <?php else: ?>
                                    <div class="avatar-user <?php echo e($u->role == 'superadmin' ? 'bg-danger bg-opacity-10 text-danger' : 'bg-primary bg-opacity-10 text-primary'); ?>">
                                        <?php echo e(strtoupper(substr($u->name, 0, 1))); ?>

                                    </div>
                                <?php endif; ?>
                                <div>
                                    <div class="fw-bold text-dark <?php echo e(!$u->is_active ? 'text-decoration-line-through' : ''); ?>"><?php echo e($u->name); ?></div>
                                    <div class="d-flex align-items-center gap-2 mt-1">
                                        <span class="text-muted" style="font-size:0.78rem; font-family:'Courier New',monospace;"><?php echo e($u->username); ?></span>
                                        <?php if($u->is_active): ?>
                        ...
                        <td>
                            ...
                            ?>
                            <span class="badge rounded-pill border py-2 px-3 d-inline-flex align-items-center gap-1 <?php echo e(!$u->is_active ? 'opacity-50' : ''); ?>"
                                  style="background:<?php echo e($roleC['bg']); ?>;color:<?php echo e($roleC['text']); ?>;border-color:<?php echo e($roleC['border']); ?>!important;font-size:0.7rem;font-weight:800;font-family:'Outfit',sans-serif;">
                                <i class="<?php echo e($roleC['icon']); ?>"></i> <?php echo e(strtoupper($u->role)); ?>

                            </span>
                        </td>
                        <td>
                            <?php if($u->department): ?>
                                <div class="d-flex align-items-center gap-2 text-muted <?php echo e(!$u->is_active ? 'opacity-50' : ''); ?>">
                                    <i class="bi bi-building opacity-50"></i>
                                    <span class="fw-semibold" style="font-size:0.85rem;"><?php echo e($u->department->name); ?></span>
                                </div>
                            <?php else: ?>
                        ...
                        <td class="text-center pe-4">
                            <div class="d-flex justify-content-center gap-2">
                                <?php if($u->id !== auth()->id()): ?>
                                <form action="<?php echo e(route('superadmin.users.toggle', $u->id)); ?>" method="POST" class="d-inline">
                                    <?php echo csrf_field(); ?> <?php echo method_field('PATCH'); ?>
                                    <button type="submit" title="<?php echo e($u->is_active ? 'Nonaktifkan' : 'Aktifkan'); ?>"
                                        class="btn-act <?php echo e($u->is_active ? 'btn-act-toggle-off' : 'btn-act-toggle-on'); ?>">
                                        <i class="bi <?php echo e($u->is_active ? 'bi-slash-circle-fill' : 'bi-check-circle-fill'); ?>"></i>
                                    </button>
                                </form>
                                <?php endif; ?>
                                <button class="btn-act btn-act-edit" title="Edit" onclick="editUser(<?php echo e($u); ?>)">
                                    <i class="bi bi-pencil-fill"></i>
                                </button>
                                <form action="<?php echo e(route('superadmin.users.destroy', $u->id)); ?>" method="POST" class="d-inline"
                                      onsubmit="return confirm('Hapus user <?php echo e(addslashes($u->name)); ?>?')">
...
<script>
    function editUser(user) {
        let url = "<?php echo e(route('superadmin.users.update', ':id')); ?>".replace(':id', user.id);
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\e_arsip\resources\views/users/index.blade.php ENDPATH**/ ?>